...
#define startTimeout() (timeout_start_ms = M5.millis())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && ((uint16_t)(M5.millis() - timeout_start_ms) > io_timeout))
...
bool VL53L0X::init(bool io_2v8)
{
  ...
  if (model_id != 0xEE) {
    ESP_LOGI("VL53L0X", "failed to read model id: %d", model_id);
    return false;
  }
  ...
  if (!getSpadInfo(&spad_count, &spad_type_is_aperture)) {
    ESP_LOGI("VL53L0X", "failed read spad");
    return false;
  }
  ...
  if (!performSingleRefCalibration(0x40)) {
    ESP_LOGI("VL53L0X", "failed performSingleRefCalibration 0x40");
    return false;
  }
  ...
  if (!performSingleRefCalibration(0x00)) {
    ESP_LOGI("VL53L0X", "failed performSingleRefCalibration 0x00");
    return false;
  }
...
bool VL53L0X::performSingleRefCalibration(uint8_t vhv_init_byte)
{
  ...
  startTimeout();
  uint8_t v;
  while (((v = readReg(RESULT_INTERRUPT_STATUS)) & 0x07) == 0)
  {
    // ESP_LOGI("VL53L0X", "RESULT_INTERRUPT_STATUS=%u", v);
    if (checkTimeoutExpired()) { return false; }
  }
